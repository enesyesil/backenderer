...
check "availability_zones" {
  assert {
    ...
    error_message = "Backenderer requires at least two availability zones."
  }
}
...
resource "aws_subnet" "private" {
  count = var.private_app_host ? length(local.azs) : 0
...
resource "aws_eip" "nat" {
  count = var.private_app_host ? 1 : 0
...
resource "aws_route_table_association" "private" {
  count = var.private_app_host ? length(aws_subnet.private) : 0
